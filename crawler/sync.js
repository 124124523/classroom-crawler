...
async function syncCourseworkToAssignments() {
  ...
  // 모든 coursework 로드 (state=PUBLISHED, due_date 있음)
  const cwSnap = await db.collection('coursework').get();
  const courseworks = cwSnap.docs
    .map(d => ({ id: d.id, ...d.data() }))
    .filter(c => c.due_date && c.state === 'PUBLISHED');

  // 기존 assignments 인덱스 (gclassroom_id → 문서)
  ...
  for (const cw of courseworks) {
    try {
      ...
      if (existing) {
        // class_id 또는 deadline 변경됐으면 업데이트
        const upd = {};
        if (classId && existing.class_id !== classId) upd.class_id = classId;
        upd.deadline = cw.due_date;
        upd.updated_at = new Date().toISOString().replace('T', ' ').slice(0, 19);
        await db.collection('assignments').doc(existing.docId).update(upd);
        skipped++;
        continue;
      }
      ...
      // 중복 검사: 같은 class_id + title + 동일 날짜
      const dupSnap = await db.collection('assignments')
        .where('class_id', '==', classId)
        .where('title', '==', cw.title)
        .get();
      const targetDate = (cw.due_date || '').slice(0, 10);
      const dup = dupSnap.docs.find(d => (d.data().deadline || '').slice(0, 10) === targetDate);
      ...
      const newId = String(++maxAssignId);
      await db.collection('assignments').doc(newId).set({
        id: maxAssignId,
        title: cw.title,
        content,
  ...
  console.log(`[sync] 완료 → 추가 ${inserted}, 스킵 ${skipped}, 조기스킵 ${skippedEarly}, 실패 ${failed}`);

...
// ── 섹션 확인 시각 저장/조회 (뱃지 영속화) ───────────────
app.get('/api/me/seen', async (req, res) => {
  if (!req.session?.user) return res.status(401).json({ message: '로그인이 필요합니다.' });
  const itemType = req.query.type;
  if (!itemType) return res.status(400).json({ message: 'type 파라미터가 필요합니다.' });
  try {
    const [rows] = await pool.query(
      'SELECT last_seen_at FROM user_last_seen WHERE user_id = ? AND item_type = ?',
      [req.session.user.id, itemType]
    );
    res.json({ last_seen_at: rows.length ? rows[0].last_seen_at : null });
  } catch (err) {
    console.error('[seen] GET 오류:', err.message);
    res.status(500).json({ message: '서버 오류' });
  }
});

app.post('/api/me/seen', async (req, res) => {
  if (!req.session?.user) return res.status(401).json({ message: '로그인이 필요합니다.' });
  const itemType = req.body.type;
  if (!itemType) return res.status(400).json({ message: 'type이 필요합니다.' });
  try {
    await pool.query(
      `INSERT INTO user_last_seen (user_id, item_type, last_seen_at)
       VALUES (?, ?, NOW())
    ...
    console.error('[seen] POST 오류:', err.message);
    res.status(500).json({ message: '서버 오류' });
  }
});
